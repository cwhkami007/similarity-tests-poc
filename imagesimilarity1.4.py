"""
Image Similarity Proof of Concept - Ver 1.4
...
def batch_compare_folders(comparison_folder, reference_folder, output_folder, methods=['VGG16', 'CLIP', 'SSIM']):
    ...
    Save top 3 matches for each comparison image and all scores
    """
    ...
    for comp_idx, comp_file in enumerate(comparison_files, 1):
        print(f"\n{'='*60}")
        print(f"Processing [{comp_idx}/{len(comparison_files)}]: {comp_file}")
        print(f"{'='*60}")
        ...
        for method in methods:
            ...
            print(f"\nTop 3 matches using {method}:")
            for i, res in enumerate(top3, 1):
                print(f"  {i}. {res['reference_image']}: {res[f'{method}_score']:.4f}")
...
if __name__ == "__main__":
    # PATHS
    comparison_folder = "C:/Users/huichris/Downloads/compbank"
    reference_folder = "C:/Users/huichris/Downloads/testbank"
    output_folder = "C:/Users/huichris/Downloads/similarity_results"
    
    # Choose methods
    methods = ['VGG16', 'TensorFlow']
